...
visualization_tm_vm_ui <- function(id) {
  ...
  fluidRow(
    ...
    box(
      ...
      # --- BOXPLOTS REST/VIBRATION datasets
      conditionalPanel(
  ...
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$generate_rest_vibration_dfs, {
      tryCatch({
        ...
        rest_vibration <- grep("rest", periods, ignore.case = TRUE, value = TRUE)
        vibration_period  <- grep("vibration",  periods, ignore.case = TRUE, value = TRUE)
        
        calc <- function(v) az %>%
          dplyr::filter(period_without_numbers %in% c(rest_vibration, vibration_period)) %>%

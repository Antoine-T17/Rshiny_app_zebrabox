  ...
  fluidRow(
    box(
      ...
      selectInput(ns("plot_type"), "Plot Type",
                  c("boxplot_vibration_rest","boxplot_cumulate","boxplot_delta","lineplot"),
                  selected = "boxplot_vibration_rest"),
      div(style="margin-bottom:20px;"),
      
      # --- BOXPLOT Vibration/Rest ---
      conditionalPanel(
        condition = cond("plot_type","boxplot_vibration_rest"),
        ...
        selectInput(ns("boxplot_vibration_rest_mode"), "Boxplot Mode", c("separated","pooled"), "separated"),
        conditionalPanel(
          condition = paste(cond("boxplot_vibration_rest_mode","pooled"), "&&", cond("plot_type","boxplot_vibration_rest")),
          textInput(ns("boxplot_vibration_rest_periods_colors"),
                    "Vibration/Rest Colors (comma-separated hex)", value = "#FFC300,#B3AAAA")
        )
      ),
      ...
      conditionalPanel(
        condition = paste(cond("plot_type","boxplot_cumulate"), "||",
                          cond("plot_type","boxplot_vibration_rest"), "||",
      ...
      tabsetPanel(
        ...
        tabPanel("Datasets",
                 selectInput(ns("dataset_type"), "Dataset Type",
                             c("Boxplot Vibration/Rest","Boxplot Cumulative","Boxplot Delta","Lineplot"),
  ...
  moduleServer(id, function(input, output, session) {
    ...
    as_widget <- function(p) {
      if (input$output_mode == "HTML") {
        pooled <- (input$plot_type == "boxplot_vibration_rest" && input$boxplot_vibration_rest_mode == "pooled") ||
    ...
    save_current_state <- function(p) {
      key <- paste(input$response_var, input$selected_zone, tolower(input$theme_switch),
                   if (input$plot_type=="boxplot_vibration_rest") input$boxplot_vibration_rest_mode
    ...
    observeEvent(input$generate_vibration_rest_dfs, {
      tryCatch({
        ...
        rest_period <- periods[grepl("rest", periods, ignore.case=TRUE)]
        vibration_period  <- periods[grepl("vibration",  periods, ignore.case=TRUE)]
        calc <- function(v) az %>%
        ...
        rv$all_zone_combined_vibration_rest_boxplots <- setNames(lapply(RESPONSE_VARS, calc), RESPONSE_VARS)
        log("✅ Vibration/Rest datasets created (17 variables).")
      }, error=function(e) log(paste("❌ Vibration/Rest generation failed:", e$message)))
    })
    ...
    generate_plot <- function(df, response_var, plot_type, boxplot_mode, lineplot_mode,
      ...
      if (plot_type == "boxplot_vibration_rest") {
        ...
        if (boxplot_mode == "separated") {
          ...
          # pooled Vibration/Rest
          sub <- tidyr::complete(sub, condition_grouped, period_without_numbers, fill = list(mean_val = NA))
          per_cols <- trimws(strsplit(input$boxplot_vibration_rest_periods_colors, ",")[[1]])
    ...
    observeEvent(input$output_mode, {
      ...
      boxmode <- if (input$plot_type == "boxplot_vibration_rest") input$boxplot_vibration_rest_mode
      else if (input$plot_type == "boxplot_delta") input$boxplot_delta_mode
      else "separated"
    ...
    observeEvent(list(
      ...
      input$boxplot_vibration_rest_mode, input$boxplot_delta_mode,
    ...
    output$save_current_figure <- downloadHandler(
      filename = function() {
        ...
        mode <- if (input$plot_type == "boxplot_vibration_rest") input$boxplot_vibration_rest_mode
        else if (input$plot_type == "boxplot_delta") input$boxplot_delta_mode
        else if (input$plot_type == "boxplot_cumulate") "separated" else input$lineplot_replicate_mode

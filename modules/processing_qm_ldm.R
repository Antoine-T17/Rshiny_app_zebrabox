# UI for Quantization Mode, Light-Dark Mode
...
# Server for Quantization Mode, Light-Dark Mode
  ...
  moduleServer(id, function(input, output, session) {
    ...
    process_zones <- function(current_data, i) {
      ...
      if (all(c(0, 2) %in% zones)) {
        ...
        num_cols <- c("frect", "fredur", "midct", "middur", "burct", "burdur", "zerct", "zerdur", "actinteg")
        for (col in num_cols) {
      ...
      processed_zones <- purrr::map(names(zone_data), ~ {
        zd <- zone_data[[.x]] %>% mutate(zone = .x) %>%
          select(any_of(c("plate_id", "period", "animal", "condition", "condition_grouped", "condition_tagged",
                          "period_with_numbers", "period_without_numbers", "zone", "start", "end", "frect", "fredur",
                          "midct", "middur", "burct", "burdur", "zerct", "zerdur", "actinteg")))
      ...
      tryCatch({
        ...
        for (i in seq_len(n_plates)) {
          ...
          if ("datatype" %in% colnames(current_data)) {
            if (any(stringr::str_detect(current_data$datatype, "quantauc"))) {
              current_data <- current_data %>% filter(stringr::str_detect(datatype, "quantauc"))
              add_console_message(sprintf("✅ Plate %d - Filtered rows where datatype contains 'quantauc'.", i))
            } else {
              add_console_message(sprintf("⚠️ Warning: Plate %d - No rows with 'quantauc' in datatype, proceeding with all raw data.", i))
            }
          } else {
            add_console_message(sprintf("⚠️ Warning: Plate %d - 'datatype' column not found, proceeding with all raw data.", i))
          }
